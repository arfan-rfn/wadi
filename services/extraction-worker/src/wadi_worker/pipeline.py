...
from wadi_worker.boundary import DiscoveredService, discover_system_services
...
class ExtractionPipeline:
    ...
    async def _extract_services(self, snapshot: Snapshot, system: System, workspace: Path) -> None:
        ...
        # Materialize EVERY repo before classifying any of them (§5.2.14).
        # Whether a module is a library depends on what the other repos of the
        # system declare, so a per-repo pass cannot see a shared internal jar
        # that lives in its own repository — the ordinary shape, and the one
        # that gave ICPC's `base` a service of its own.
        ...
        for repo in system.repos:
            normalized = normalize_repo_source(repo.source)
            checkout = checkouts[normalized]
            discovered = by_repo.get(normalized, [])
            if not discovered:
