...
object TypeShapes {
  ...
  /** Nodes left to spend on one shape (§5.2.15).
    ...
    */
  private final class Budget(private var remaining: Int) {
    def spend(): Boolean = {
      if (remaining <= 0) false
      else { remaining -= 1; true }
    }
  }
  ...
  private def build(
    ...
    budget: Budget = new Budget(MaxNodes)
  ): ujson.Obj = {
    ...
    bindings.get(raw.name) match {
      case Some(bound) if bound.name != raw.name =>
        return build(cpg, bound, path, depth, bindings - raw.name, budget)
      case _ =>
    }
    ...
    if (raw.array)
      return node(
        "array",
        raw.name,
        element = Some(build(cpg, raw.copy(array = false), path, depth, bindings, budget))
      )
    if (UnwrapOne.contains(simple) && raw.args.nonEmpty)
      return build(cpg, raw.args.head, path, depth, bindings, budget)
    if (ArrayLike.contains(simple)) {
      val element = raw.args.headOption.map(build(cpg, _, path, depth, bindings, budget))
      return node("array", raw.name, element = element)
    }
    if (MapLike.contains(simple)) {
      val value = raw.args.lift(1).map(build(cpg, _, path, depth, bindings, budget))
    ...
    resolveTypeDecl(cpg, raw.name) match {
      ...
      case Some(typeDecl) =>
        ...
        // Charged per EXPANDED object, not per node visited: scalars, cycles
        // and unresolved terminals are already leaves and cost nothing to
        // emit. What runs away is objects opening more objects.
        else if (!budget.spend()) node("truncated", raw.name)
        else {
          val fields = wireMembers(typeDecl)
            .map(fieldShape(cpg, _, path + typeDecl.fullName, depth - 1, bindings, budget))
  ...
  private def fieldShape(
    ...
    val shape = parseTypeText(declaredText)
      .map(build(cpg, _, path, depth, bindings, budget))
