...
**Complete (T0–T7), measured end to end in §5.2.11.** Contracts 1.20.0, export 2.10.0. *Two of this phase's own conclusions were wrong and were corrected by measurement rather than argument* — T2's zero was blamed on a binding failure that does not exist, and T5's largest bucket was a mislabel of a correct terminal. Both survived because an aggregate moved in the expected direction; both fell the moment a single instance was read against its source. That is the standing lesson: **a count is a hypothesis until one of its members is opened.** *Rejected: a T8 against the 77 remaining `unresolved-receiver` sites* — they are javasrc2cpg sentinel types, a frontend limitation wadi cannot close from here, and 77 of 8,600 unbound calls does not earn a tranche ahead of Phase 3.
