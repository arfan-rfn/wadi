...
*M3 bytecode-oracle whitelist (enumerated up front — every entry is a javac desugaring where bytecode branch/loop counts legitimately diverge from the source-level graph):* short-circuit `&&`/`||` (one source condition, N bytecode branches); ternary (one statement, one bytecode branch); switch-on-string (double dispatch: `hashCode` switch + `equals` chain); enhanced-`for` (iterator `hasNext` loop); try-with-resources (duplicated close/finally paths, `Throwable#addSuppressed` branches); `finally` (block duplicated per exit path); lambda bodies (`invokedynamic` + synthetic method — compared as separate methods, matching the CPG's `<lambda>N` shape); `assert` (`$assertionsDisabled` branch); string concat (`makeConcatWithConstants`, no branches). The oracle diffs per-method branch/loop counts; every divergence must be a whitelisted desugaring, a recorded limitation here, or a bug.
...
### Phase 2.6 — Control-flow fidelity — ← current
