...
*Client APIs* — handled: RestTemplate (all methods), `@FeignClient` (name/value/url/path, Spring-MVC mappings), WebClient (T1), **RestClient** (T2 — same fluent shape as WebClient, `.uri(...)` is the sink; the yas 34-invisible-sites lesson, closed), **`RequestEntity`-form `exchange`** (T2 — verb + URL recovered from the entity's builder chain off the call site, inline or via method-local assignment). Planned T2 (remaining): `@HttpExchange` interfaces, `RestTemplateBuilder.rootUri`/`baseUrl` splits (relative call-site paths must report "base undetermined", never a fabricated absolute), Feign interface inheritance, raw-Feign `@RequestLine`, `@RequestMapping(method=…)` on Feign methods. Planned T2 (lower): AsyncRestTemplate, OAuth2RestTemplate/RestOperations, JDK `HttpClient`, OkHttp, Apache HttpClient. Out of scope until demanded by coverage data: Retrofit, Unirest (test-scoped clients like RestAssured classify by source-set and are excluded from production topology).

*URL construction* — handled: literals, `+` concat (flattened, T1), `String.format` (`%s`/`%d`), locals/fields (owner-scoped, T1), `static final` constants, field-level `@Value`, interprocedural returns (hop-budgeted), same-method literal constant maps, Lombok getter bridge, **`UriComponentsBuilder` chains** (T2 — base + `path`/`pathSegment` steps resolved in call order; `queryParam`/`fragment`/`build*`/`encode` are identity-neutral and skipped with a trace note; unmodelled steps like `host`/`scheme` contribute an honest hole, never a guess), **`URI.create`** (T2 — transparent wrapper, argument unwrapped). Planned T2 (remaining): parameter/constructor-level `@Value`, `@ConfigurationProperties` beans, ternary/switch expressions, `StringBuilder`, `String.concat/join/formatted`, `MessageFormat`, field-held/`Map.of` constant maps, `System.getenv` with literal defaults, builder-in-local `UriComponentsBuilder` chains (v1 models the single-expression chain — the dominant idiom). Honestly undecidable: URLs from response data (HATEOAS hrefs), reflection/SpEL-built URLs — reason-coded, never guessed.
...
**Precise definitions (recorded with the implementation).** *Denominator* — internal (non-external), **concrete** methods (abstract/interface stubs have nothing to walk and their implementations count separately — but an *empty* concrete method is still production code and counts) whose name does not start with `<` (excludes operators, constructors/initializers, and frontend-synthesized lambda wrappers — the same filter the closure BFS applies, so T4 widens numerator and denominator *together*), from the service's **own** sources (`wadi-libs/` staged library sources excluded — a library's methods would duplicate into every dependent's denominator; same rationale as the unreachable-inventory exclusion, §5.2.6). *Numerator* — the endpoint-reachable closure intersected with the denominator set (a stub in the closure counts in neither). Both counts are computed in-CPG at export time and travel export → `ServiceBoundary.analysis_coverage` (worker) → coverage-report section (stitcher); a service without the fact (extraction failed, pre-metric snapshot) reports `null` counts, never zero (P10). Libraries are excluded from the per-service listing entirely — they are not analysis units (§5.2.6).
